    ...
    switch (this.fase)  {
        case 'N':
          ...
          res => {
                ...
                this.lastLogistica = res['lastnumber'];
                this. creaworklogistica(this.lastLogistica);

           // eliminata questa modalità di recupero ultimo id
...
//        if(resp['rc'] === 'ok') {
 // this.loadwFile(idsett);
      ...
      async  registranewWorkSettore(workSettore: WorkSettore)  {
          ...
          let rc = await this.worksettoreService.create(workSettore).subscribe(
            resp => {
                  ...
                  if(resp['rc'] === 'ok') {
                    this.registraFilebySettore(workSettore.idLogistica, workSettore.idSettore);
                  }
             },
            error => {
  ...
  async registraFilebySettore(idLogistica: number, isSettore: number) {

    console.log('registraFilebySettore  --- appena entrato');
    let rc = await this.wFileService.getAll().subscribe(
        resp => {
              ...
              if(resp['rc'] === 'ok') {
                ...
                for(let wfila of this.wfile) {
                   ...
                   // this.workSettore.id = wsett.id;
                    this.workFila.idLogistica = this.lastLogistica;
                    this.workFila.idSettore = isSettore;
                    this.workFila.dfila = wfila.descrizione;
                    x += 1;
                        console.log(`registraFilebySettore: ------- pronto per inserire ---------------->  ${JSON.stringify(this.workFila)}  ------   contatore c: ${x} `);
        ...
        error => {
             alert('sono in registraFilebySettore');
             console.log('registraFilebySettore - errore: ' + error);
             this.type = 'error';
             this.Message = error.message;
             this.showNotification(this.type, this.Message);
         });
...
/*
      metodi originali  creati per gestire workSettore - workFile   2023/10/31
}
