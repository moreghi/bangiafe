...
import { faPlusSquare, faSearch, faSave, faUserEdit, faMinus, faPlus, faWindowClose, faTrash, faInfo,
         faInfoCircle, faList, faTicketAlt, faLocationArrow, faChair, faAddressBook, faUsers, faCheckDouble} from '@fortawesome/free-solid-svg-icons';
 ...
 */
 ...
 // variabili per notifica esito operazione con Notifier
  ...
  //   this.textUser = this.messa.demessa;
     ...
     this.stato = +localStorage.getItem('StatoAssegni');
     localStorage.removeItem('StatoAssegni');


     alert('passato dal padre- stato: ' + this.stato);
...
showNotification( type: string, message: string ): void {
  ...
  emissione(biglietto: Biglietto) {

    console.log('Biglietto -----  sono in emissione: ' + JSON.stringify(biglietto));

   //  this.route.navigate(['biglietto/' + biglietto.id + '/edit']); non visualizza bene la mappa

   // this.route.navigate(['biglietto/' + biglietto.id + '/edit/xx']) // mappa dettaglio originale -- non usata

     this.route.navigate(['biglietto/' + biglietto.id + '/edit/BG']);
  }
