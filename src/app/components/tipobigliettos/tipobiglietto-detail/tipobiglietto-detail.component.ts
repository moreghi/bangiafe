...
/*
    ...
    if(this.tipoticket == 1) {
      this.flagpu = 'PU';
    }
    if(this.tipoticket == 2) {
      this.flagpu = 'NU';
    }  */
    localStorage.removeItem('tipoBiglietto');


    console.log('-----  tipobiglietto-detail  -->  goApplication - anno: ---- ' + this.anno + ' flagpu: ' + this.flagpu.toUpperCase());
    ...
    // creo le wTaglie in base alla selezione fatta
    // erano originariamente dentro a goApplication
    this.loadStati();
  ...
  async loadEvento(id: number) {
    ...
    response => {
      if(response['rc'] === 'ok') {
        ...
        if(this.npostiAssegnati == 0) {
          console.log(' ---------------------------------  uscita per posti = 0')
          this.stato = 0;
    ...
    async loadManifestazione(id: number) {
      let rc = await  this.manifestazioneService.getbyId(id).subscribe(
        response => {
          if(response['rc'] === 'ok') {
            console.log('loadManifestazione: ' + JSON.stringify(response['data']));
    ...
    async Loadtipobiglietto(id: number) {
        ...
        response => {
          if(response['rc'] === 'ok') {
            console.log('.............. Loadtipobiglietto: ' + JSON.stringify(response['data']));
          ...
          this.showNotification( this.type, this.Message);
           ...
           async  loadwEventotaglabiglietto(id: number, stato: number) {
            console.log('.........  lettura biglietto per combo ..................  loadwEventotaglabiglietto - Appena entrato' + id + ' stato: ' + stato );
                ...
                resp => {
                  console.log('                     trovati wEvento...' + JSON.stringify(resp['data']) + ' rc: ' + resp['rc']);
                       if(resp['rc'] === 'ok') {
                         ...
                         /*
                         for(const wtaglia of this.wEventoTagliaBigliettiSelected) {
                          if(wtaglia.flagpu.toUpperCase() !== this.flagpu.toUpperCase()) {
                           console.log('wtaglia.flagpu trovata: ' + JSON.stringify(wtaglia));
                               this.cancellaTaglianonAttiva(wtaglia);
                           }
                         }
*/
             ...
             async  loadwEventotaglabigliettoSelected(id: number, stato: number) {
              console.log('...........................  loadwEventotaglabigliettoSelected - Appena entrato' + id + ' stato: ' + stato + ' flagpu: ' + this.flagpu);
                  ...
                  resp => {
                    console.log('                     trovati wEvento..selected ' + JSON.stringify(resp['data']) + ' rc: ' + resp['rc']);
                         if(resp['rc'] === 'ok') {
                          ...
                          /*   da canecllare
                           for(const wtaglia of this.wEventoTagliaBigliettiSelected) {
                           if(wtaglia.flagpu.toUpperCase() !== this.flagpu) {
                            console.log('wtaglia.flagpu trovata: ' + JSON.stringify(wtaglia));
                                this.cancellaTaglianonAttiva(wtaglia);
                            }
                          }  */
    ...
    release() {
      ...
      //  creare i bigliettida loop t_tipo_biglietto
      console.log('.-.-.-.-.-.-.-.-.-.-.-.-.-.  creo i biglietti .-.-.-.-.-.-.-.-.-.-.-.-.-.');
      this.createAllBiglietti(this.evento);
      // alla fine cancellare w.eventotagli ( per evento)
      console.log('.-.-.-.-.-.-.-.-.-.-.-.-.-.  cancello wtagliaeventi per evento .-.-.-.-.-.-.-.-.-.-.-.-.-.  ' + this.evento.id);
      this.cancellawtagliaBiglietto(this.evento.id);
      this.aggiornaEvento(this.evento)
    }


    async  createAllBiglietti(evento: Evento)  {
      let rc = await this.tipobigliettoService.getbyevento(evento.id).subscribe(
        res => {
                if(res['rc'] === 'ok') {
                  this.tipobiglietti = res['data'];
                  for(const tipobigl of this.tipobiglietti) {
                    this.tipobiglietto = tipobigl;

                    this.inx=1;
                    this.ultimoemesso = this.tipobiglietto.ultimoemesso;
                    while ( this.inx < this.tipobiglietto.ntot + 1) {
                      // creo il biglietto
                      this.biglietto = new Biglietto();
                      this.biglietto.evento = this.evento.id;
                      this.biglietto.importo = this.tipobiglietto.importo;
                      this.ultimoemesso += 1;
                      this.biglietto.numero = this.ultimoemesso;
                      this.biglietto.serie = this.tipobiglietto.serie;
                      this.biglietto.tipo = this.tipobiglietto.idtipotaglia;
                      this.biglietto.key_utenti_operation = 1;
                      this.creaBiglietto(this.biglietto);
                    this.inx = this.inx + 1;
                    }
                  }
              }
    ...
    async   cancellawtagliaBiglietto(id: number)   {
      let rc = await  this.wEventoTagliaBigliettoService.deleteAll(id).subscribe(
        response => {
            if(response['rc'] === 'ok') {
    //                non faccio nulla
        console.log('-----> cancellate taglie su wEventotaglia per evento: ' + id);
            }
        },
        error =>
        {
    ...
    async   creaBiglietto(biglietto: Biglietto)  {
      let rc = await  this.BigliettoService.create(biglietto).subscribe(
        response => {
            if(response['rc'] === 'ok') {
    //                non faccio nulla
        console.log('-----> creato biglietto: ' + biglietto.numero);
            }
        },
        error =>
        {
          ...
          if( this.tipobiglietto.ntot > this.npostiDisp) {
           ...
           if( this.selectedTaglio === 0 ) {
            ...
            return;
                  ...
                  console.log('conferma - fase: ' + this.fase) + ' ------------>  ora devo creare i biglietti se senza logistica';
