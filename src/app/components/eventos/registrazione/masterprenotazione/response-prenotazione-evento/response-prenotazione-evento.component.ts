...
//  originale  ----------------- getRegConfirmbyTokenProm
  ...
  async  confermaAlleventoPosto(token: string) {
    ...
    response => {
        ...
        this.eventoposti = response['data'];
        for(const posto of this.eventoposti) {
          posto.stato = 3;    // Prenotato e da rilasciare a Bandiera gialla
  ...
  async  confermaeventoPosto(posto: EventoPosto) {
    ...
    response => {
           if(response['rc'] === 'ok') {
            // creare prenotazione  necessario a backoffice di bandiera gialla per emettere biglietto
            this.prenotazione = new Prenotazione();
            this.prenotazione.cognome = posto.cognome;
            this.prenotazione.nome = posto.nome;
            this.prenotazione.datapren = posto.dataconf;
            this.prenotazione.email = posto.email;
            this.prenotazione.idevento = posto.idEvento;
            this.prenotazione.persone = 1;
            this.prenotazione.telefono = posto.cellulare;
            this.prenotazione.token = posto.token;
            this.creaPrenotazione(this.prenotazione);
          }
    },
    error => {
  ...
  async inviaemaildiConferma(prenotazione: Prenotazione) {
    ...
    let rc = await  this.prenotazioneService.sendConfermaPrenotazione(prenotazione).subscribe(
      response => {
             if(response['rc'] === 'ok') {
              // inviata email ad ogni partecipante per ritiro biglietto
