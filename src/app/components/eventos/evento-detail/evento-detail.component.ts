...
import { EventonotepopComponent } from '../../../components/popups/eventonotepop/eventonotepop.component'; // visualizzazione locandina
// nuovo datepiker


//   video esempio datepicker ngx-bootstrap       https://www.youtube.com/watch?v=edaN6iCcqP4
...
onSelectedLogistica(selectedValue: number) {
  //  alert('selezionato: ' + selectedValue);
        ...
        if(selectedValue !== 0) {
          ...
          this.loadLogistica(selectedValue);
      ...
      let rc = await this.logisticaService.getbyId(id).subscribe(
          resp => {
                ...
                if(resp['rc'] === 'ok') {
                  ...
                  this.evento.nposti = this.logistica.nposti;
                  this.npostiLogistica = this.logistica.nposti;
                }
                if(resp['rc'] === 'nf') {
                  this.logistiche = this.logistichenull;
                  this.pathimage = '';
...
conferma() {
  ...
  if(this.evento.nposti === 0) {
    this.type = 'error';
    this.Message = 'Inserire il numero di posti';
    this.alertSuccess = false;
    this.showNotification(this.type, this.Message);
    return;
  }
...
async createAllFile(eventoSettore: EventoSettore) {
 ...
 // console.log('-- 1EventoSettore  per creaqre Fila   -------createAllFile  -------   appena entrato +  ---- eventoSettore: ' + JSON.stringify(eventoSettore));
  let rc = await this.logfilaService.getbylogisticaeSettore(eventoSettore.idLogistica, eventoSettore.idSettore).subscribe(
    resp => {
       ...
       //       console.log(' 4----------   createAllfile ---- fila: ' + JSON.stringify(fila))
                ...
                this.creafila(eventoSettore.dsettore,   this.eventoFila, c);
          //      console.log('c6 -------- CreateAllFile ---- pronto per create: c: ' + c + ' ....... ' + JSON.stringify(this.eventoFila))
              }
              this.CreateeventoSettfilaPosto(eventoSettore.idEvento);
         }
      },
      error => {
  ...
  async creafila(dsettore: string, eventoFila: EventoFila, c: number)  {
    let xx = 0;
    ...
    response => {
        if(response['rc'] === 'ok') {
          this.eventoFila1 = response['data'];
           ...
           this.creapostidellafila(response['ultimoid'], dsettore);
          }
   },
   error => {
...
async createAllPosti(fila: EventoFila, dsettore: string) {
 console.log('....................................................  CreateAllPosti  --- Appena entrato')
    ...
    for (let i = fila.nstart; i < fila.nend +1; i++) {
      ...
      this.eventoPosto.desposto = dsettore + ' -- ' + fila.dfila + ' -- Posto: ' + i;
  ...
  //                non faccio nulla
     ...
     error => {
           alert('sono in createAllPosti');
            console.log('createAllPosti - errore: ' + error);
           this.type = 'error';
           this.Message = error.message;
           this.showNotification(this.type, this.Message);
       });
...
async CreateeventoSettfilaPosto(id: number) {
  let c = 0;
 // console.log('-- 1EventoSettore  per creaqre Fila   -------createAllFile  -------   appena entrato +  ---- eventoSettore: ' + JSON.stringify(eventoSettore));
  let rc = await this.eventofilaService.getbyevento(id).subscribe(
    resp => {
        if(resp['rc'] === 'ok') {
         ...
         //     console.log(' 2 ----------   createAllFile ---- logfile: ' + JSON.stringify(this.logFile))
            ...
            for(const fila of this.eventoFile) {
                ...
                this.eventosettfilaposto.idLogistica = fila.idLogistica;
                this.eventosettfilaposto.idEvento = fila.idEvento;    //  leggere ultimo creato
                this.eventosettfilaposto.idSettore = fila.idSettore;
                this.eventosettfilaposto.idFila = fila.id;
                this.eventosettfilaposto.postoStart = fila.nstart;
                this.eventosettfilaposto.postoEnd = fila.nend;
                this.eventoFila.key_utenti_operation = 1;
                 this.creaeventosettfilaposto(this.eventosettfilaposto);
          //      console.log('c6 -------- CreateAllFile ---- pronto per create: c: ' + c + ' ....... ' + JSON.stringify(this.eventoFila))
              }
      ...
      error => {
           alert('sono in CreateeventoSettfilaPosto');
           console.log('CreateeventoSettfilaPosto - errore: ' + error);
           this.type = 'error';
           this.Message = error.message;
           this.showNotification(this.type, this.Message);
       });
    ...
    async creaeventosettfilaposto(eventosettfilaposto: Eventosettfilaposti)  {
      let rc = await  this.eventosettfilapostiService.create(eventosettfilaposto).subscribe(
        response => {
            if(response['rc'] === 'ok') {
                //  non faccio nulla
              }
       ...
       error => {
             alert('sono in creaeventosettfilaposto');
              console.log('creaeventosettfilaposto - errore: ' + error);
             this.type = 'error';
             this.Message = error.message;
             this.showNotification(this.type, this.Message);
         });
...
openNotepopup() {
  ...
  const ref = this.modalService.open(EventonotepopComponent, {size:'lg'});
  ref.componentInstance.selectedUser = this.evento;
  ref.result.then(
       (yes) => {
         console.log('Click YES');
...
// effettuo i controlli generali prima di creare evento
...
// test da non fare perchè tutti con tipobiglietto = 0  ------  eliminare
/*
if(this.evento.tipobiglietto === 0) {
          this.type = 'error';
          this.Message = 'Effettuare la Gestione Prezzi per poter completare la registrazione';
...
*/
       ...
       //       alert('Procedo con creazione evento e poi con la gestione prezzi');
       //        return;
 ...
 //  passi originali della creazione evento fino al 02/05/2023
            ...
            /*
               ...
               qui faccio la gestione prezzo unico o a fascie e quando finisco
               facco route.navigate[evento/edit/:idevento/idManifestazione]
               e qui concludo le attivitò impostcndo a operativo l'evento
            */
...
//  viene eseguito correttamente
      async  CreaFascieTagliBiglietti(flagpu: string) {
                  ...
                  //    console.log('CreaFascieTagliBiglietti: ' + JSON.stringify(resp['data']));
                      if(resp['rc'] === 'ok') {
                        ...
                        for(const taglio of this.tagli) {
                            if(taglio.id > 0 ) {
                              if(taglio.flagpu.toUpperCase() == flagpu.toUpperCase()) {
...
async creaNewEvento(evento: Evento) {
  let rc = await this.eventoService.create(evento).subscribe(
    res => {
      if(res['rc'] === 'ok') {
        this.eventocreato = true;
        this.CreaFascieTagliBiglietti(this.flagpu);           //  viene eseguito correttamente
